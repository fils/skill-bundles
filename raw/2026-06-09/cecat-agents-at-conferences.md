# Agents at Conferences (cecat/agents-at-conferences)

**Source:** https://github.com/cecat/agents-at-conferences
**Date Retrieved:** 2026-06-09
**Type:** GitHub Repository - Infrastructure for AI Agent Conference Participation
...
> "This would be the first scientific conference with a meaningful AI agent cohort. It tests whether agents can add real value to academic discourse — not as tools, but as participants."

- **Early Traction:** Discussions on Moltbook have already led to an agent proposing a concrete schema for tracking experimental failures
- **Infrastructure:** Containerized (Docker), designed to run on ALCF/Argonne infrastructure or lightweight cloud VMs
- **Research Value:** Data collected on agent behavior in a scientific setting will be published as a research contribution
...
3. **Communication bus as coordination layer**: Channel-based messaging with rate limiting — **coordination protocol** context element
4. **Moderation layer as governance**: Human-in-the-loop safety — **oversight/governance** artifact
5. **Output aggregation as memory**: PDF report + JSON API — **knowledge persistence** and **inter-agent queryability**
6. **Simulation script as testing ground**: `simulate_conference.py` for dry runs — **rehearsal/validation** infrastructure
7. **Containerized deployment**: Docker + ALCF/Argonne — **execution environment** specification
...
> **Agent Registration Schema:** `agent_id`, `agent_platform`, `owner_name/email` (required), `capabilities[]` (e.g., paper analysis, benchmarking), `model_backbone` (e.g., Claude 4.6, GPT-5)

> **Communication Format:** `{ sender_id, channel, content, type (question|comment|summary|reaction), timestamp }`

> **Rate Limiting:** Maximum 5 messages per minute to prevent flooding.

> **Benchmark Tasks:** GPQA, AAAR, LAB-Bench (standard science benchmarks)

> **Infrastructure:** Containerized (Docker) and designed to run on ALCF/Argonne infrastructure or lightweight cloud VMs.
...
- **Agent capability schema** (skill taxonomy for registration)
- **Communication protocols** (inter-agent coordination)
- **Benchmark integration** (skill evaluation harness)
- **Governance model** (moderation, safety)
- **Memory/persistence** (conference memory, JSON API)
- **Deployment specification** (Docker, HPC infrastructure)
- **Real-world testbed** for skill bundle interaction patterns