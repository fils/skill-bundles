...
**Type:** GitHub Repository - Domain-specific skill library with formal validation

---

## Summary

**ClawBio** is the first **bioinformatics-native skill library** for AI agents. Built on **OpenClaw**, it provides a **local-first, privacy-focused, and reproducible** framework for genomic analysis. It bridges the gap between stochastic LLM reasoning and rigorous bioinformatics execution.

This is an **excellent example of a domain-specific skill bundle** — it packages 88 skills with formal specification layers, validation infrastructure, reproducibility bundles, and a reference genome for deterministic testing.
...
| **nf-core Wrappers** | Pipeline | Production wrappers for `scrnaseq`, `rnaseq`, and `sarek` (variant calling) |
...
This bundle includes **rich formal context elements**:
...
- **This is a formal reproducibility artifact — a key context element**
...
6. **Galaxy Bridge**: Access to 8,000+ external tools — demonstrates **external tool registry mapping** as a context element

---

## Key Quotes

> "ClawBio introduces a **Specification Layer** to AI agents. Instead of allowing an LLM to improvise code (which leads to hallucinations in critical areas like dosage), ClawBio uses a machine-readable contract."

> "**Specification-First:** Domain expertise is encoded in `SKILL.md` files, not model weights."

> "**Reproducible:** Generates a `reproducibility/` bundle containing:
> * `commands.sh`: The exact replay command.
> * `environment.yml`: Conda environment snapshot.
> * `checksums.sha256`: SHA-256 verification for outputs."

> "As of **v0.5.0 (April 2026)**, ClawBio includes a systematic validation infrastructure:
> * **AD Ground Truth:** 34 Alzheimer's genes across 3 evidence tiers.
> * **Mock API Server:** Deterministic endpoints for offline CI testing.
> * **Performance:** The public leaderboard shows **92.3% (168/182)** tests passing across audited skills."

> "**Reference Genome: The Corpasome** — ClawBio uses a real, fully open human genome (Manuel Corpas, CC0) for all demo data."

---

## Reproducibility Assessment

**Very High.** The bundle includes:
- Exact reference genome with DOI (immutable, versioned test data)
- Per-execution reproducibility bundles (commands, environment, checksums)
- Mock API server for deterministic offline testing
- Ground truth validation sets (34 Alzheimer's genes, 3 tiers)
- Public leaderboard with quantitative pass rates
- Deterministic environment via `uv`
- Scaffolding tool for consistent skill creation