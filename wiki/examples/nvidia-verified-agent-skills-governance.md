...
date: 2026-07-14
sources: ["https://developer.nvidia.com/blog/nvidia-verified-agent-skills-provide-capability-governance-for-ai-agents/", "https://github.com/NVIDIA/skills"]
skills_included: ["SkillSpector security scanning", "Cryptographic skill signing", "Skill cards (trust metadata)", "Daily catalog sync", "Verification flow (automated + human review)"]
context_elements: ["Skill cards (ownership, dependencies, limitations, verification status)", "Cryptographic signing (skill.oms.sig)", "Security scanning (SkillSpector)", "agentskills.io spec compliance", "Cross-platform portability (Claude Code, Codex, Cursor)"]
...
- Complements [[skillguard-permission-framework]] (runtime) and [[skillfortify-formal-verification-supply-chain]] (static analysis) — NVIDIA focuses on publication/distribution layer
- Skill cards extend [[prov-agent-unified-provenance]] (provenance) to commercial skill distribution
- Cryptographic signing parallels [[supply-chain-agentic-factory-in-toto]] (SLSA/in-toto) for skills
- Built on [[agentskills-io-specification]] — same SKILL.md across harnesses
- SkillSpector = automated security scanning (complements [[skillsieve-malicious-skill-detection]])
